 ...
 * maar exclusief verwerking n.a.v. een 'hit'.
 \*********************************************************************************************/
boolean ProcessEvent(unsigned long IncomingEvent, byte Direction, byte Port, unsigned long PreviousContent, byte PreviousPort)
  {
  byte x;
  boolean CommandForThisNodo=CheckEventlist(IncomingEvent,Port) || NodoType(IncomingEvent)==NODO_TYPE_COMMAND;
  ...
  PrintEvent(IncomingEvent,Port,Direction);  // geef event weer op Serial

  // houdt bij wat de busy status van andere Nodo's is.
  if(((IncomingEvent>>16)&0xff)==CMD_BUSY) // command
    {
    x=(IncomingEvent>>24)&0xf; // unit
    if(((IncomingEvent>>8)&0xff)==VALUE_ON) // Par1
      {
      BusyNodo|=(1<<x);
  ...
  if(CommandForThisNodo)
    ...
    if(Hold)
      {
      if((IncomingEvent&0xffffff00)==command2event(CMD_DELAY,0,0) ||
         (IncomingEvent&0xffffff00)==command2event(CMD_WAITBUSY,0,0)   )
        {
      ...
      if(QueuePos<EVENT_QUEUE_MAX)
        {
        QueueEvent[QueuePos]=IncomingEvent;
      ...
      ProcessEvent2(IncomingEvent,Direction,Port,PreviousContent,PreviousPort);
    }
...
boolean ProcessEvent2(unsigned long IncomingEvent, byte Direction, byte Port, unsigned long PreviousContent, byte PreviousPort)
  {
  ...
  if(NodoType(IncomingEvent)==NODO_TYPE_COMMAND)
    { // Er is een geldig Commando binnengekomen       
    if(!ExecuteCommand(IncomingEvent,Port,PreviousContent,PreviousPort))
      {
      EventlistDepth--;
      return false;
      }
    ...
    for(x=1; x<=Eventlist_MAX && Eventlist_Read(x,&Event_1,&Event_2); x++)
      {
      if(CheckEvent(IncomingEvent,Event_1,Port))
        {
        ...
        if(NodoType(Event_2)==NODO_TYPE_COMMAND) // is de ontvangen code een uitvoerbaar commando?
          {
          if(!ExecuteCommand(Event_2, VALUE_SOURCE_EVENTLIST,IncomingEvent,Port))
            {
            EventlistDepth--;
          ...
          if(Event_1!=command2event(CMD_COMMAND_WILDCARD,0,0))
            {
            if(!ProcessEvent2(Event_2,VALUE_DIRECTION_INTERNAL,VALUE_SOURCE_EVENTLIST,IncomingEvent,Port))
              {
              EventlistDepth--;
              return true;
              }
